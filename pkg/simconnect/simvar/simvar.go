...
type SimVar struct {
	c *eventsdata.EventsData
}

// New creates a SimVar client backed by the provided eventsdata.Session.
func New(c eventsdata.Session) *SimVar {
	return &SimVar{c: eventsdata.New(c)}
}
...
func (sv *SimVar) GetOnce(ctx context.Context, def *Definition[any], obj core.ObjectID) (any, error) {
	return eventsdata.RequestDataOnce(ctx, sv.c, def, obj)
}
...
func (sv *SimVar) GetByTypeOnce(ctx context.Context, def *Definition[any], radiusMeters uint32, t core.SimObjectType) (Update[any], error) {
	return eventsdata.RequestDataByTypeOnce(ctx, sv.c, def, radiusMeters, t)
}
...
func (sv *SimVar) Set(ctx context.Context, def *Definition[any], obj core.ObjectID, value any, flags core.DataSetFlag) error {
	...
	return eventsdata.SetDataOnSimObject(sv.c, def, obj, value, flags)
}
...
func (sv *SimVar) Subscribe(ctx context.Context, def *Definition[any], obj core.ObjectID, period core.Period, opts ...Option) (<-chan Update[any], error) {
	return eventsdata.SubscribeData(ctx, sv.c, def, obj, period, opts...)
}
...
func GetOnce[T any](ctx context.Context, c eventsdata.Session, def *Definition[T], obj core.ObjectID) (T, error) {
	return eventsdata.RequestDataOnce(ctx, eventsdata.New(c), def, obj)
}

func GetByTypeOnce[T any](ctx context.Context, c eventsdata.Session, def *Definition[T], radiusMeters uint32, t core.SimObjectType) (Update[T], error) {
	return eventsdata.RequestDataByTypeOnce(ctx, eventsdata.New(c), def, radiusMeters, t)
}

func Set[T any](ctx context.Context, c eventsdata.Session, def *Definition[T], obj core.ObjectID, value T, flags core.DataSetFlag) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
	}
	return eventsdata.SetDataOnSimObject(eventsdata.New(c), def, obj, value, flags)
}

func Subscribe[T any](ctx context.Context, c eventsdata.Session, def *Definition[T], obj core.ObjectID, period core.Period, opts ...Option) (<-chan Update[T], error) {
	return eventsdata.SubscribeData(ctx, eventsdata.New(c), def, obj, period, opts...)
}
