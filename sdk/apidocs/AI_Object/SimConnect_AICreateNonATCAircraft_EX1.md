SimConnect\_AICreateNonATCAircraft\_EX1

## SimConnect\_AICreateNonATCAircraft\_EX1

The **SimConnect\_AICreateNonATCAircraft\_EX1** function is used to create an aircraft that is not flying under ATC control (so is typically flying under VFR rules). This function can be used with both _legacy_ SimObjects, and **modular** SimObjects.
...
| _InitPos_ | Specifies the initial position, using a `SIMCONNECT_DATA_INITPOSITION` structure. | Integer |
...
Refer to the remarks for [SimConnect\_AICreateEnrouteATCAircraft\_EA1](SimConnect_AICreateEnrouteATCAircraft_EX1.htm).
